...
def create_app():
    ...
    def suggest_transfers(squad_ids, players, fixture_difficulty, bank=0):
        ...
        for weak in squad_sorted:
            ...
            candidates = [p for p in players.values() if p["element_type"] == pos and p["id"] not in excluded_ids and p["status"] == "a" and p["minutes"] > 90]
            for c in candidates:
    ...
    def build_preseason_data(players, fixture_difficulty, teams_map):
        ...
        scout_picks = sorted([p for p in all_players if p["status"] == "a" and p["price"] <= 8.5], key=lambda p: p["score"], reverse=True)[:8]
    ...
    def api_comparison(league_id):
        ...
        try:
            ...
            finished_gws = [ev["id"] for ev in bootstrap["events"] if ev["id"] >= from_gw and ev["finished"]]
    ...
    def api_league_demo():
        ...
        def mock_bench_order(bench):
            ...
            return [{"order": i + 1, "name": p["name"], "team": p["team_name"], "position": p["position"], "form": p["form"], "price": p["price"], "score": round(p["form"] * 2, 2)} for i, p in enumerate(outfield)]
                ...
                "teams": [
                    {
                        ...
                        "squad": best_squad,
                        "transfers": [],
                        "captain_suggestion": {"id": 30, "name": "B.Fernandes", "team": "MUN", "position": "MID", "form": 6.7, "price": 12.0, "score": 28.0, "reason": "Top scorer of the 2025/26 season with 235 points"},
                        "bench_order": [
